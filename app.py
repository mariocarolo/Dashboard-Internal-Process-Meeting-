...
def todos_to_df(todos: list[dict], with_owner: bool = False) -> pd.DataFrame:
    rows = [{
        ...
        "waiting":     "Sim" if t.get("waiting_on_them") else "Não",
        "owner":       t.get("owner", ""),
        "category":    t.get("category", CATEGORIES[0]),
        "updated":     t.get("updated", ""),
    } for t in todos]
    cols = ["id", "company", "notes", "description", "status", "waiting",
            "owner", "category", "updated"]
...
def make_grid(todos: list[dict], with_owner: bool = False, key: str = "g") -> tuple:
    ...
    df = todos_to_df(todos, with_owner=with_owner)
    ...
    gb.configure_column("owner",    hide=not with_owner)

    if with_owner:
        gb.configure_column(
            "owner", headerName="Responsável", width=105, editable=True,
    ...
    gb.configure_column(
        "waiting", headerName="⏳ Aguardando", width=110, editable=True,
        cellEditor="agSelectCellEditor",
        cellEditorParams={"values": ["Não", "Sim"]},
        cellStyle={"fontSize": "0.74rem", "color": "#8a3a0a"},
    )
    gb.configure_column(
    ...
    # Resolve selected_rows regardless of version format
    sel_id = None
...
def sync_grid(grid_df, scope_ids: list[str]) -> bool:
    ...
    for row in rows:
        ...
        # Text fields: only update if grid value is non-empty and actually different
        ...
        # Waiting boolean
        nv_w = str(row.get("waiting", "Não")).strip() == "Sim"
...
.sg-person-meta { font-size: 0.7rem; color: rgba(255,255,255,0.75);
...
/* ── Company stats ── */
.sg-co-stats { display: flex; gap: 1.1rem; flex-wrap: wrap; margin-top: 0.35rem; }
.sg-co-stat { font-size: 0.66rem; color: #a09080; font-family:'JetBrains Mono',monospace; }
.sg-co-stat b { color: #4a3e38; }
...
def grid_action_row(selected_id: str | None, owner_key: str) -> None:
    """Mark Done / Delete buttons for the selected row."""
...
def tab_by_person(todos: list[dict]) -> None:
    ...
    ca, cb, cc, _ = st.columns([2.2, 2.2, 2.2, 6])
    with ca:
    ...
    for owner, meta in PEOPLE.items():
        ...
        bg = meta["bg"]
        st.markdown(f"""
<div class="sg-person-header" style="background:{bg}">
  <div class="sg-person-initial">{owner}</div>
  <div class="sg-person-meta">{open_c} open &nbsp;·&nbsp; {len(person_todos)} total</div>
</div>""", unsafe_allow_html=True)
        ...
        st.markdown('<div class="sg-hint">Clique para selecionar &nbsp;·&nbsp; '
                    'Duplo clique na célula para editar &nbsp;·&nbsp; Enter ou Tab para salvar</div>',
                    unsafe_allow_html=True)
...
def tab_all_todos(todos: list[dict]) -> None:
    ...
    for cat in CATEGORIES:
        cat_items = [t for t in fil if t.get("category", CATEGORIES[0]) == cat]
        ...
        st.markdown('<div class="sg-hint">Duplo clique para editar qualquer célula</div>',
                    unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
...
<div class="sg-stats">
  ...
  <div class="sg-stat"><div class="sg-stat-value" style="color:#692730;font-size:0.95rem">{top}</div><div class="sg-stat-label">Mais Pendências</div></div>
    ...
    for company in companies:
        ...
        c_open  = sum(1 for t in ct if t["status"] == "Open")
        c_inp   = sum(1 for t in ct if t["status"] == "In Progress")
        c_w     = sum(1 for t in ct if t.get("waiting_on_them"))
        c_done  = sum(1 for t in ct if t["status"] == "Done")
        owners  = ", ".join(sorted({t["owner"] for t in ct}))

        with st.expander(f"{company}  ·  {len(ct)} itens  ({c_open} open)", expanded=False):
            st.markdown(f"""
<div class="sg-co-stats" style="margin-bottom:0.7rem">
  <div class="sg-co-stat">Open: <b>{c_open}</b></div>
  <div class="sg-co-stat">In Progress: <b>{c_inp}</b></div>
  <div class="sg-co-stat">Aguardando: <b>{c_w}</b></div>
  <div class="sg-co-stat">Done: <b>{c_done}</b></div>
  <div class="sg-co-stat">Responsáveis: <b>{owners}</b></div>
</div>""", unsafe_allow_html=True)

            scope_ids = [t["id"] for t in ct]
            gdata, sel_id = make_grid(ct, with_owner=True, key=f"g_co_{company[:8]}")
            if gdata is not None and sync_grid(gdata, scope_ids):
                st.rerun()
            grid_action_row(sel_id, f"co_{company[:6]}")
...
def tab_summary(todos: list[dict]) -> None:
    ...
    lines = [f"INTERNAL PROCESS MEETING — WEEKLY SUMMARY", f"Gerado em: {today}", ""]
